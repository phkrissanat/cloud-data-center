...
async function loadState(){
  ...
  const own=state.sections.filter(s=>s.owner_id===state.user.id);options($('section'),own);options($('root-section'),state.sections);document.querySelector('[data-tab=import]').hidden=state.user.role!=='admin';await showRoots();
...
async function updateAutoScan(){
 ...
 $('auto-status').textContent=result.running?'กำลังสแกนไฟล์อัตโนมัติ…':!result.sections.length?'ยังไม่ได้ผูกโฟลเดอร์: ให้ Admin ผูกกับ Section ครั้งเดียวในเมนูสแกนโฟลเดอร์':'สแกนอัตโนมัติทุก 30 วินาที • ล่าสุด '+new Date(result.last_finished*1000).toLocaleTimeString('th-TH')+(issues?' • พบ '+issues+' รายการที่ต้องตรวจสอบ':'');

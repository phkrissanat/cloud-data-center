...
$('login').onsubmit=async event=>{event.preventDefault();$('login-button').disabled=true;try{const raw=$('endpoint').value.trim();const url=new URL(raw||location.origin);if(url.username||url.password||url.search||url.hash||url.pathname!=='/')throw new Error('กรอกเฉพาะที่อยู่เซิร์ฟเวอร์ ไม่ใส่ path');if(url.protocol!=='https:'&&!(url.protocol==='http:'&&['localhost','127.0.0.1'].includes(url.hostname)))throw new Error('เซิร์ฟเวอร์ออนไลน์ต้องใช้ HTTPS');endpoint=url.origin;const result=await json('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('username').value,password:$('password').value})});token=result.token;epoch++;$('password').value='';await loadState();$('login-panel').hidden=true;$('workspace').hidden=false;await search();tab('files');message('เข้าสู่ระบบแล้ว');const match=location.hash.match(/^#document=(\d+)$/);if(match)await history(Number(match[1]));}catch(e){message(e.message,true);}finally{$('login-button').disabled=false;}};

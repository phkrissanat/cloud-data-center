...
async function search(append=false){
  ...
  const data=await json(`/api/documents?q=${encodeURIComponent($('query').value)}&l1=${encodeURIComponent($('filter-l1').value)}&l2=${encodeURIComponent($('filter-l2').value)}&l3=${encodeURIComponent($('filter-l3').value)}&offset=${offset}`);
...
async function loadFolderLevel(level){
  const select=level===0?$('filter-l1'):level===1?$('filter-l2'):$('filter-l3');
  ...
  const old=select.value;select.replaceChildren(...(level===2?[new Option('all','')]:[]));for(const o of opts)select.add(new Option(o,o));
  if(opts.includes(old))select.value=old;
}
async function loadFolders(){await loadFolderLevel(0);await loadFolderLevel(1);await loadFolderLevel(2);}
$('filter-l1').onchange=async()=>{$('filter-l2').value='';$('filter-l3').value='';try{await loadFolderLevel(1);await loadFolderLevel(2);await search();}catch(e){message(e.message,true);}};
$('filter-l2').onchange=async()=>{$('filter-l3').value='';try{await loadFolderLevel(2);await search();}catch(e){message(e.message,true);}};
$('filter-l3').onchange=async()=>{try{await search();}catch(e){message(e.message,true);}};

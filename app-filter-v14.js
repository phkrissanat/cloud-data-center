...
$('query').name='q-'+Math.random().toString(36).slice(2);
...
async function loadState(){
  ...
  $('demo-note').hidden=true;$('logout').hidden=false;$('refresh').hidden=state.user.role==='reader';$('connection-status').hidden=state.user.role==='reader';$('mode').hidden=state.user.role==='reader';document.querySelector('nav').hidden=state.user.role==='reader';$('files-title').hidden=state.user.role==='reader';$('files').classList.toggle('bare',state.user.role==='reader');
...
async function search(append=false){
  ...
  const blank=!$('query').value.trim()&&!$('filter-l1').value&&!$('filter-l2').value&&!$('filter-l3').value&&!$('filter-l4').value;
  if(state.user.role==='reader'&&blank){$('count').textContent='';$('more').hidden=true;return;}
  const data=await json(`/api/documents?q=${encodeURIComponent($('query').value)}&l1=${encodeURIComponent($('filter-l1').value)}&l2=${encodeURIComponent($('filter-l2').value)}&l3=${encodeURIComponent($('filter-l3').value)}&l4=${encodeURIComponent($('filter-l4').value)}&offset=${offset}`);
...
$('login').onsubmit=async event=>{event.preventDefault();$('login-button').disabled=true;try{if(!resolvedBackend&&!isLocalPage)throw new Error('กดเปิดเว็บจาก Control Dashboard เพื่อเชื่อมต่อ Backend');endpoint=new URL(resolvedBackend||location.origin).origin;const result=await json('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('username').value,password:$('password').value})});token=result.token;try{localStorage.setItem('section-backend',endpoint);}catch{}connectionLost=false;epoch++;$('password').value='';await loadState();$('login-panel').hidden=true;$('workspace').hidden=false;await loadFolders();if(state.user.role!=='reader')await search();tab('files');if(state.user.role!=='reader')message('เข้าสู่ระบบแล้ว');const match=location.hash.match(/^#document=(\d+)$/);if(match)await history(Number(match[1]));}catch(e){message(e.message,true);}finally{$('login-button').disabled=false;}};
$('logout').onclick=async()=>{try{await api('/api/logout',{method:'POST'});}catch{}epoch++;token='';state=null;rows=[];resetUpload();$('results').replaceChildren();$('versions').replaceChildren();$('history').hidden=true;$('notifications-list').replaceChildren();$('workspace').hidden=true;$('login-panel').hidden=false;$('who').textContent='';$('demo-note').hidden=false;$('logout').hidden=true;$('refresh').hidden=true;$('connection-status').hidden=true;$('connection-status').textContent='';$('mode').hidden=true;$('message').textContent='';};
...
const LEVEL_SELECTS=['filter-l1','filter-l2','filter-l3','filter-l4'];
const LEVEL_LABELS=['กลุ่มสินค้า','หมวดสินค้า','รุ่นสินค้า','รายละเอียด'];
async function loadFolderLevel(level){
  const select=$(LEVEL_SELECTS[level]);
  const params=new URLSearchParams({level});
  if(level>=1&&$('filter-l1').value)params.set('l1',$('filter-l1').value);
  if(level>=2&&$('filter-l2').value)params.set('l2',$('filter-l2').value);
  if(level>=3&&$('filter-l3').value)params.set('l3',$('filter-l3').value);
  const opts=await json('/api/folder-options?'+params);
  const old=select.value;select.replaceChildren(new Option(LEVEL_LABELS[level],''));for(const o of opts)select.add(new Option(o,o));
  if(opts.includes(old))select.value=old;
}
async function loadFolders(){for(let lvl=0;lvl<4;lvl++)await loadFolderLevel(lvl);}
$('filter-l1').onchange=async()=>{$('filter-l2').value='';$('filter-l3').value='';$('filter-l4').value='';try{await loadFolderLevel(1);await loadFolderLevel(2);await loadFolderLevel(3);await search();}catch(e){message(e.message,true);}};
$('filter-l2').onchange=async()=>{$('filter-l3').value='';$('filter-l4').value='';try{await loadFolderLevel(2);await loadFolderLevel(3);await search();}catch(e){message(e.message,true);}};
$('filter-l3').onchange=async()=>{$('filter-l4').value='';try{await loadFolderLevel(3);await search();}catch(e){message(e.message,true);}};
$('filter-l4').onchange=async()=>{try{await search();}catch(e){message(e.message,true);}};
...
async function updateAutoScan(){
 ...
 try{const result=await json('/api/scan-status');
  ...
  if(result.last_finished&&result.last_finished!==autoFinished){autoFinished=result.last_finished;await loadFolders();if(!$('files').hidden&&state.user.role!=='reader')await search();}

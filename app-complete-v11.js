...
let allAccounts=[],editingAccount=null,rootMappings=[];
async function loadReaders(){allAccounts=await json('/api/admin/accounts');$('readers-list').replaceChildren();for(const u of allAccounts){const row=el('article',undefined,'card');row.append(el('h3',u.username+' • '+u.role),el('p',(u.active?'เปิดใช้งาน':'ปิดใช้งาน')+(u.section?' • '+u.section:'')),button('แก้ชื่อ / เปิดปิด',()=>{editingAccount=u.id;$('edit-account').hidden=false;$('edit-account-name').value=u.username;$('edit-account-active').checked=!!u.active;}),button('ตั้งรหัสใหม่',()=>{resetReaderId=u.id;$('reset-reader').hidden=false;$('reset-reader-title').textContent='ตั้งรหัสใหม่: '+u.username;}));$('readers-list').append(row);}const sections=state.sections.filter(s=>!s.owner.startsWith('system-'));options($('transfer-section-id'),sections);options($('remap-section'),sections);$('transfer-owner').replaceChildren(...allAccounts.filter(u=>u.active&&u.role!=='admin').map(u=>new Option(u.username,u.id)));selectTransfer();rootMappings=await json('/api/roots');}
...
$('edit-account').onsubmit=async e=>{e.preventDefault();if(!confirm('บันทึกการเปลี่ยนบัญชี? บัญชีนี้จะต้องเข้าสู่ระบบใหม่'))return;try{const r=await json('/api/admin/accounts/'+editingAccount,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('edit-account-name').value,active:$('edit-account-active').checked})});$('edit-account').hidden=true;if(r.sign_in_again){location.reload();return;}await loadReaders();message('บันทึกบัญชีแล้ว');}catch(e){message(e.message,true);}};
